...
This directory contains a Copilot Chat skill manifest (`SKILL.md`) and a helper script (`run_ci.py`) to trigger GitHub Actions workflows for a branch using the `gh` CLI.

Quick usage (dry-run by default):

```bash
python .github/skills/run-ci/run_ci.py --branch fix/some-change --workflow docker.yml --dry-run
```

To actually push and run:

```bash
python .github/skills/run-ci/run_ci.py --branch fix/some-change --workflow docker.yml
```
...
- `terraform.yml` — Runs `terraform plan` (and optionally `apply`) against `infra/`.
	...
	- How to run manually (example):
		...
		python .github/skills/run-ci/run_ci.py --branch fix/some-change --workflow terraform.yml --dry-run
		```
