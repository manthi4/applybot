...
class TestBuildSearchQueries:
    ...
    @patch("applybot.discovery.query_builder.complete")
    def test_llm_generates_queries(self, mock_complete, mock_profile):
        mock_complete.return_value = GeneratedQueries(
    ...
    @patch("applybot.discovery.query_builder.complete")
    def test_llm_result_truncated_to_max_queries(self, mock_complete, mock_profile):
        mock_complete.return_value = GeneratedQueries(
    ...
    @patch("applybot.discovery.query_builder.complete")
    def test_llm_failure_falls_back_to_defaults(self, mock_complete, mock_profile):
        mock_complete.side_effect = RuntimeError("API down")
        result = build_search_queries(mock_profile)
        assert result == DEFAULT_QUERIES[:6]

    @patch("applybot.discovery.query_builder.complete")
    def test_prompt_includes_profile_info(self, mock_complete, mock_profile):
        mock_complete.return_value = GeneratedQueries(queries=["q1"])
        build_search_queries(mock_profile, max_queries=1)

        call_args = mock_complete.call_args
        prompt = call_args[0][2]  # third positional arg: provider, model, prompt
        assert "Test User" in prompt or mock_profile.name in prompt
        assert "Python" in prompt or "skills" in prompt.lower()

    @patch("applybot.discovery.query_builder.complete")
    def test_profile_with_empty_skills(self, mock_complete):
        profile = MagicMock()
        ...
        profile.experiences = []
        profile.preferences = {}
        mock_complete.return_value = GeneratedQueries(queries=["ml jobs"])
